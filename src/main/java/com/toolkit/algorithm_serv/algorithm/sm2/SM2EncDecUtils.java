...
public class SM2EncDecUtils {
    ...
    //数据加密
    public static String encrypt(byte[] publicKey, byte[] data, boolean oldVer) throws IOException {
    ...
    //数据解密
    public static byte[] decrypt(byte[] privateKey, byte[] encryptedData, boolean oldVer) throws IOException {
         ...
         */
        byte[] c1Bytes = null, c3 = null, c2 = null;
        int c2Len = 0;
        if (oldVer) {
            // 分解加密字串 C1 | C2 | C3
            c1Bytes = Util.hexToByte(data.substring(0, 130));
            c2Len = encryptedData.length - 97;
